...
public class GildedRoseTest {
    ...
    public void BackstagePassQualityShouldBeZeroIfSellInIsLessThanZero() {
        int sellIn = 0;
        Item[] items = new Item[]{new Item("Backstage passes to a TAFKAL80ETC concert", sellIn, 15)};
        GildedRose app = new GildedRose(items);
        int days = 3;
        for (int i = 0; i < days; i++) {
            app.updateQuality();
            sellIn--;
            assertEquals("Backstage passes to a TAFKAL80ETC concert, " + sellIn + ", 0", app.items[0].toString());
        }
    }
    ...
    public void NormalItemQualityShouldntBeNegative() {
        ...
        for (int i = 0; i < days; i++) {
            app.updateQuality();
            sellIn--;
            assertEquals("Normal item, " + sellIn + ", 0", app.items[0].toString());
        }
    }
}
